 ...
 */
static void
rewind_file_range(const char *path, off_t begin, off_t end, bool trunc)
{
	char		buf[BLCKSZ];
	...
	for (i = 0; i < map->narray; i++)
	{
		...
		switch (entry->action)
		{
			...
			case FILE_ACTION_COPY:
				rewind_file_range(entry->path, 0, entry->newsize, true);
				break;
			...
			case FILE_ACTION_COPY_TAIL:
				rewind_file_range(entry->path, entry->oldsize, entry->newsize, false);
				break;
...
execute_pagemap(datapagemap_t *pagemap, const char *path)
{
	...
	{
		offset = blkno * BLCKSZ;
		rewind_file_range(path, offset, offset + BLCKSZ, false);
